export default async function handler(req, res) {
    ...
    if (!code) {
        const authUrl = `https://login.microsoftonline.com/${AZURE_CONFIG.tenantId}/oauth2/v2.0/authorize?client_id=${AZURE_CONFIG.clientId}&redirect_uri=${encodeURIComponent(AZURE_CONFIG.redirectUri)}&response_type=code&scope=${encodeURIComponent('https://graph.microsoft.com/.default')}&state=random`;
